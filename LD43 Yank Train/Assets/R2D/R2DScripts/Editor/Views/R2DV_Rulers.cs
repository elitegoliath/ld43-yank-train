...
namespace R2D
{
	...
	public class R2DV_Rulers 
    {
        ...
        void DrawGuidesLock()
        {
            if (state.displayGuides)
            {
                Texture2D lockTexture = state.lockGuides ? resources.lockActive : resources.lockInactive;
                drawing.DrawTexture(lockTexture, 
                                    Mathf.CeilToInt(viewPixelFrame.topRight.x) - lockTexture.width + 1,
                                    0,
                                    lockTexture.width, 
                                    lockTexture.height);
            }
